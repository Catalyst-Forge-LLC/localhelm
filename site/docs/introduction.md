...
| Plan, then write | `enroll`, `unenroll`, `bump`, `commit`, `pull`, `push`, `export`, `publish`, `cascade`, `archive`, `plugin`, `land`, `ship`, `global` | Prints a plan. `--apply` writes. `publish`, `push`, `ship`, and `global` need named ids |
