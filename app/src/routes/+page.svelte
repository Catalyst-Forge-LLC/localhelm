<script lang="ts">
	...
	const {
		...
		confirmItemKeys: () => confirmItemKeys,
		confirmPhases: () => confirmPhases,
		setConfirmPhases: (phases) => {
			confirmPhases = phases;
		},
		confirmMessages: () => confirmMessages,
		setConfirmMessages: (messages) => {
			confirmMessages = messages;
		},
		...
		confirmDrafting: () => confirmDrafting,
		setConfirmDrafting: (ids) => {
			confirmDrafting = ids;
		},
		confirmDraftNotes: () => confirmDraftNotes,
		setConfirmDraftNotes: (notes) => {
			confirmDraftNotes = notes;
		},
		setConfirmDraftHint: (hint) => {
			confirmDraftHint = hint;
		},
		error: () => error,
		setError: (message) => {
			error = message;
		...
		lastPublishPlan: () => lastPublishPlan,
		setLastPublishPlan: (rows) => {
		...
		publishAuthHint: () => publishAuthHint,
		setPublishAuthHint: (hint) => {
			publishAuthHint = hint;
		},
	});
	...
	<main class:fill-pane={tab === 'today'}>
		{#if tab === 'today'}
			<TodayBoard
				...
				{needPublishIds}
				{needPushIds}
				attentionEmpty={attentionRows.length === 0 && cascadeOnlyRows.length === 0}
				filteredEmpty={filteredAttentionRows.length === 0 && filteredCascadeRows.length === 0}
				...
				{portFamilyCards}
				{portsNeedingYou}
				onRefreshRow={(id) => void refreshRows([id])}
				onCommit={(ids) => void startCommit(ids)}
				onPublish={startPublish}
				onPush={(ids) => void startPush(ids)}
				onCascade={(id) => void startCascade(id)}
				onLand={startLand}
				onSyncEngine={(plugin, ids) => void startPluginJob(plugin, 'sync', ids, 'Sync engine')}
				onSetTab={setTab}
