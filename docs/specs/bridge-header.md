...
**Stations + body (2026-09-15):** Operator asked to go brighter and carry the HUD to tabs / boards / tables. Chart grid is on the whole `.shell`. The tab rail is a darker veil (hull fade, not a solid plate) under a cyan waterline so stations sit off the paper. Stations are tracked uppercase with a cyan active underline. Panels get corner ticks; writes stay gold.  
...
- **Desktop:** lamps and gauges sit on one row. The keel (remotes / npm / live step) spans **under both**, not inside the lamp bay.
- **≤ 64rem:** hide gauges, wake spark, and the desk Refresh / Pull / Push / Activity plates. Those four move into the locker. Ident + lamps + menu stay on the glass; keel is full width under.
