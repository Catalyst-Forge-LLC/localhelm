...
**F4. Publish confirm: commits since last npm.** We already have local version + `git`. `git log --oneline` from the tag or `v$local`..HEAD (skip if no tag). Stops “what am I shipping?” The confirm modal already lists rows.
