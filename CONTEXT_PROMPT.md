...
- Header (d70): brand is **LocalHelm** plus need-only chips (unpublished, dirty, pins behind, missing, npm errors). Refresh, Pull, and Push stay in the bar. The menu holds fleet path, serve line, npm whoami, remotes, Deck, Copy brief, Fetch remotes, Write inventory JSON, and plugin on/off. Working… / errors sit in a reserved rail so the page does not jump.
