...
- Plugin host: `localhelm.plugin.mjs` on an enrolled project, default export `{ id, label, board }`. A file that does not match, or a `board()` that throws, is a fault on the keel and in the locker; the other plugins still load (d144). FilePress plugin lives in the filepress checkout (Sites). **LocalSlip** plugin lives in the `localslip` checkout (Ports tab, plugin id `localslip`). **xFacts** plugin lives in the `x-facts` checkout and lists the enrolled fleet. **FeatureFacts** plugin lives in the `feature-facts` checkout (`id: featurefacts`). Add repos scans a folder and inits an empty register through that plugin; Helm does not write `.featurefacts/` itself. Do not reimplement leases, observe, or firewall. Site push is `git push origin <branch>` only — never `--force`. Plugin actions may set `icon` (Lucide); LocalHelm falls back by action id (`sync` / `push` / `ship` / `start` / `stop`).
- Product pairing (d55/d56): keep two products/dashboards. **LocalSlip** is the slip (`localslip` on npm; **localslip.dev** primary). Do not fold the lease kernel into Helm.
...
- `publish`: plan then named-id `--apply`. If the checkout has a GitHub Actions `workflow_dispatch` that runs `npm publish` with OIDC/`--provenance`, Helm does not call npm here — it finishes bump/commit/push, then shows a github.com Actions link so you can run **Publish**. Laptop npm auth is `localhelm auth` + a granular automation token (Bypass 2FA) in the **user** `~/.npmrc` for packages that still publish locally. That still works through ~Jan 2027; npm is moving to trusted/staged publish after that. Never store the token in this repo.
- Dashboard writes are one button: click Push / Publish / Sync / Bump / etc., the modal shows the plan, then Confirm applies or Close if nothing to do. CLI still prints a plan and needs `--apply`. Today and Fleet share the same write list (`fleetWriteIds`): **Commit** (dirty tree), Publish, Push, Write pins. Today golds the first. Leftover blockers stay as pills. Toolbar + checks stay for bulk. **Commit** lists the dirty files, drafts a message via ollanet→Ollama (or a fallback), and you edit/approve. Confirm is `git add` + `git commit` only — no push. `.env`, `*.pem`, `id_rsa`, `credentials.json`, and `.npmrc` are listed as skip. Named ids; never commit the whole fleet in one apply. **Bump** writes root `package.json` and any `APP`/`TOOL`/`SKILL_FACTS.md` version lines, then commits those files (`Helm: bump <pkg> to <ver>.`). Publish’s commit step uses the same file list — it must not leave facts dirty after a cut. Other dirty files stay local. No tag, no push, no publish. Write pins is only offered when a clean consumer has a registry pin behind — not for `link:`/`file:` or dirty dependents.
- Dashboard IA: **Today** (default, needs-you + FilePress + Ports snapshots), **Fleet** (enroll + git/npm table), **Sites** (FilePress jobs), **Ports** (LocalSlip leases). Same product can appear on three tabs — **also on** chips jump and check that id. Header and tabs stay pinned; each tab pane scrolls in the remaining viewport. Activity is a right drawer (Escape / Close) persisted at `.localhelm/activity.json`. No second ship list. FilePress site names that match a fleet id are labeled as sites, not packages. Plugin apply only offers write ids when the plan marks `writes: true`. Long board notes live in the ? tip, not the section head. Ports subtabs: **Leases** (LocalSlip filter chips: Listen / Bind / Lease / Firewall), **Stacks** (one row per family with its own Start/Stop), **Observed** (Bind filters). Observed stays read-only. Claim/release stay on `localslip`.
- Header (d70/d137/d140/d143): stacked lockup **local** / **HELM** (Oxanium / Syne). Need-only lamps (publish, push, dirty, pins behind, missing, npm errors) match Today’s write buttons — not digest `unpublishedAhead`. No manifest / empty `projects` is **No fleet** (opens Add), not All quiet; keel is `No fleet yet · Add projects on Today`. Desktop keeps Refresh, Pull, Push, Activity, and gauges on the bar. At ≤64rem those conn controls (and gauges) move into the locker; at ≤40rem lamps hide and keel stays under the ident. The menu also holds fleet path, serve line, npm whoami, remotes, Deck, Copy brief, Fetch remotes, Write inventory JSON, Main/Demo, and plugin on/off. Working… / errors sit in a reserved rail so the page does not jump.
- **Deck** (d76; was visitor face d57–d60): tiles live at **`/deck`**. A non-loopback hit on `/` redirects there; `/visitor` 302s to `/deck`. Operator board stays on `/`; menu **Deck** opens it from the inside. `localhelm serve` binds **0.0.0.0**; Vite `allowedHosts: true` so `*.ts.net` is not 403’d. Open links use the phone Host. Favicons load from a LAN/Tailscale IP when that Host is a DNS name (other Vite apps 403 `*.ts.net`). Write APIs stay loopback-only except `GET /api/visitor`.
- Dashboard URL state (Exec Foundry pattern): `replaceState` only — never push history for UI. Query params on `/`: `tab` (omit `today`), `ports` (omit `leases` for the subtab; `stacks` / `observed`), `activity=1` when the drawer is open, `fleet` / `sites` / `leases` as CSV checked ids. Defaults omitted so URLs stay short. npm whoami stays in `sessionStorage` only. One-time migrate from old `sessionStorage` tab/pane/activity keys, then clear those keys.
...
- After a successful publish, `npm view` latest can still show the old version. Status now checks whether the local version exists on the registry (`npm view name@version`) before calling it unpublished. A publish plan that bumps says it is cutting a new version.
...
- **Land** (Sites + Today site cards, CLI `localhelm land <site-id>...`): named sites only — never an implicit land-all. Sites toolbar is **Land (N)** for checked rows and **Sync engine (N)** for checked rows that are behind; Today **Land N** / **Sync engine** cover sites that need an engine write or a finished ship. One confirm lists every site’s steps; the dashboard continues remaining sites after one failure (CLI still stops). A failed ship is not recorded as landed. Site only: FilePress Sync (`getfilepress` pin) → site Push → Ship. Does not pull, push, or publish the filepress package or a matching fleet package (use Fleet Publish for npm). Never `--force`. Ship is skipped when the site tree fingerprint matches `.localhelm/land-ships.json` (recorded after a successful Land or Sites Ship). Plan is one FilePress `land` call for all named ids (same idea as Sync/Ship) — not one request per site.
...
- Cheap surfaces (draft): `docs/specs/cheap-surfaces.md`. H1–H5 landed: family stacks, look cards, `PORT`/`HOST` on confirm, `localhelm brief` + Copy brief, soft archive (`.localhelm/archive.json`, hide not delete; Fleet hide also drops `id-site` from Today Land; Sites has Archive / Archived / Restore), family start/stop, Save guess. Today Ports is one column with titled Stacks and Down or conflicted lists.
...
- Quiet sites, cross-walk chips (Fleet / Site / Ports), copy path/URL, activity titles jump to a row, recipe health + Save all guesses. Tippy (Exec Foundry pattern) holds button labels and long facts.

### Session 17 — 2026-08-25

- Ports row actions stay on one line (flex, no 3-column wrap). Stack chips are labeled and check a family. Sites/Ports board notes moved into the ? tip. Fleet needs you is Push / Publish / Write pins when those writes are allowed. Cross-walk chips say **also on** and use tab names.

### Session 18 — 2026-08-26
...
- Today is a four-pane board (Needs you, Looks, Sites, Ports). Each pane scrolls. Refresh sits next to the project id. Needs you is fleet writes; Looks is Ports facts. Needs you filters: All / Publish / Cut / Push (`?need=`). The pane head offers bulk **Commit dirty (N)**, **Publish (N)**, and **Push (N)** when those writes exist. Each still opens a plan you confirm. Write pins stay per publisher.
...
- FilePress site at `site/` (modeled on LocalSlip): pages + `/docs` mount, FilePress id `localhelm`, lease `localhelm-site` on **5206**, `pnpm ship` → Cloudflare Pages project `localhelm`. Need `site/static/favicon.png` as well as `logo.png` / `favicon.svg` — getfilepress 0.1.18 prerender 404s a missing PNG. Operator board shot is `site/static/dashboard.jpg` (home + `/docs/dashboard`). README points at `https://localhelm.dev/dashboard.jpg` (not GitHub raw; repo is private; not in the npm tarball).
...
- Public slogan is **Control panel for local development** (d62). The local fleet is the apps and sites you keep, plus ports. Do not say “fleet” alone on the brochure. Public copy links [LocalSlip](https://localslip.dev).
- npm README logo must be an unpkg URL (`https://unpkg.com/localhelm/site/static/logo.png`). The GitHub repo is private, so a relative or raw.githubusercontent.com src 404s on npmjs. Do not put the AppFacts viewer hash under a Nutrition label heading — npm strips the link and leaves an empty heading. AppFacts stays on the site footer.
...
- Checkout `vite dev` must not set `ssr.noExternal` (Vite 8/Rolldown also rejects `false`). Vite 8 inlines CJS `ignore` and `/api/status` 500s with `module is not defined`. Bundle-everything stays on `vite build` only.
- Publishing this checkout (`prepublishOnly` → `app` build) used to make Vite full-reload and drop the publish modal. Watch ignores `dashboard/`, `dist/`, and `.svelte-kit` output/generated. Each publish id is saved to sessionStorage; a reload restores the result list plus leftover GitHub links. One failed npm id no longer skips the rest.
...
- Looks cards group by title. One row lists every fact for that id (recipe, cwd, enroll vs lease). **Site has no port lease** is only for FilePress / `-site` ids that have neither that name nor `id-site` claimed (parked still counts). A fleet package that has a site is not a missing-port problem. **Lease** plans `localslip claim <name>-site --or-next` through the LocalSlip plugin (Helm does not write the registry). Hint still counts facts. Other jumps: recipe/split → Ports or Stacks, missing folder on an enrolled row → Fleet, lease with no fleet row → Add.
- Add projects hides folders that are already enrolled. The list is only new candidates. Scan shows the same Knight Rider bar as Confirm, with the live busy label. Nested FilePress `site/` folders are not scan candidates (enroll the package; the site lives under it). Dashboard imports `isNestedSitePath` from `$lib/scanPaths` — never `$lib/scan`, which collides with the Node walker and pulls `node:os` into Vite.
...
- CodeQL `js/incomplete-sanitization` on `quoteWinArg` (`src/lib/serve.ts`): `replace(/"/g, '\\"')` left backslashes unescaped. Windows serve spawn now uses CommandLineToArgvW quoting (double `\\` before a `"`).
- `catalyst-forge` (and any checkout that only has the IngotVault `backup` remote) is not “missing origin” in the parser — there is no remote named origin; `main` tracks `backup`. Git line says **backup only**. Push is not offered for ahead-of-backup. Helm still never pushes `backup`.
- Commit confirm draft hint must count the planned repo list, not `Object.keys(messages)`. The first Ollama fill made the hint “1 of 1” / “drafted 1 message” and bind:messageById could write that one key back over the fallbacks. Progress is “Asking Ollama… N of M (id)”. Messages stay one-way (oninput merges).
- Today FilePress “waiting on Land” is engine-behind or a failed ship. Sites **Land (N)** was the checked-and-allowed count (select-all → 21 of 26; Keep local drops the rest). Label is **Land checked (N)** so it does not look like Today’s need list.
- Today bulk writes are **Publish (N)** / **Push (N)** / **Commit dirty (N)** — not “Push ahead”.
...
- Looks no longer says **Enrolled, no port lease** for a fleet package that has a FilePress site (skill + site does not need a package slip). Missing **site** slips get **Lease**, which claims `name-site` via LocalSlip `--or-next`.
...
- Bridge lockup is stacked **local** / **HELM**. Narrow widths fold Refresh, Pull, Push, Activity, and gauges into the locker so the header does not collide (d143). Keel (remotes / npm) is a well plate that hangs just above lamps + gauges, same bottom rounding as body panels.
...
### Session 54 — 2026-09-25

- FeatureFacts Add repos (d148): same scan-and-tick modal as Add sites. Init writes an empty register into each ticked repo and skips one that already has `.featurefacts/`. The board lists repos that have a register. Scan, Check, and Report run in that repo.
