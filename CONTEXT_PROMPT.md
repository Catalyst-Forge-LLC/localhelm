...
- Plugin host: `localhelm.plugin.mjs` on an enrolled project. FilePress plugin lives in the filepress checkout (Sites). **LocalSlip** plugin lives in the sibling checkout (Ports tab, plugin id `localslip`; GitHub is `localslip`, local folder may still be `localberth` until renamed). Do not reimplement leases, observe, or firewall. Site push is `git push origin <branch>` only — never `--force`. Plugin actions may set `icon` (Lucide); LocalHelm falls back by action id (`sync` / `push` / `ship` / `start` / `stop`).
