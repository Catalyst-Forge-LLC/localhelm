...
- Visitor face (d57/d58): `localhelm serve` binds **0.0.0.0** by default so the phone can hit Helm without a flag. `--host` / `--hosts` alone (Vite-style) also means all interfaces; `--host 127.0.0.1` stays loopback-only. On a LAN/Tailscale Host the board is a tile grid of listening Ports leases past loopback. Open links use that Host + the lease port. Helm does not peek or proxy favicons. Write APIs stay loopback-only. The LocalSlip dashboard is still the slip; this is the same phone menu on the wheel.
