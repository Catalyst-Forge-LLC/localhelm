...
- Plugin host: `localhelm.plugin.mjs` on an enrolled project, default export `{ id, label, board }`. A file that does not match, or a `board()` that throws, is a fault on the keel and in the locker; the other plugins still load (d144). FilePress plugin lives in the filepress checkout (Sites). **LocalSlip** plugin lives in the `localslip` checkout (Ports tab, plugin id `localslip`). **xFacts** plugin lives in the `x-facts` checkout and lists the enrolled fleet. **FeatureFacts** plugin lives in the `feature-facts` checkout (`id: featurefacts`). Do not reimplement leases, observe, or firewall. Site push is `git push origin <branch>` only — never `--force`. Plugin actions may set `icon` (Lucide); LocalHelm falls back by action id (`sync` / `push` / `ship` / `start` / `stop`).
