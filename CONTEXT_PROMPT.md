...
- Plugin host: `localhelm.plugin.mjs` on an enrolled project. FilePress plugin lives in the filepress checkout (Sites). **LocalSlip** plugin lives in the `localslip` checkout (Ports tab, plugin id `localslip`). **xFacts** plugin lives in the `x-facts` checkout and lists the enrolled fleet. Do not reimplement leases, observe, or firewall. Site push is `git push origin <branch>` only — never `--force`. Plugin actions may set `icon` (Lucide); LocalHelm falls back by action id (`sync` / `push` / `ship` / `start` / `stop`).
