...
- **AI/LLM:** none in the product core. Optional local **Ollama** (`127.0.0.1:11434`) drafts commit messages only; git owns the file list. `LOCALHELM_OLLAMA_URL` / `LOCALHELM_OLLAMA_MODEL` override. If Ollama is down, the modal still opens with a fallback you edit.
...
- Plugin host: `localhelm.plugin.mjs` on an enrolled project. FilePress plugin lives in the filepress checkout (Sites). **LocalSlip** plugin lives in the `localslip` checkout (Ports tab, plugin id `localslip`). Do not reimplement leases, observe, or firewall. Site push is `git push origin <branch>` only — never `--force`. Plugin actions may set `icon` (Lucide); LocalHelm falls back by action id (`sync` / `push` / `ship` / `start` / `stop`).
...
- Dirty repos get **Commit** on Today and Fleet (and a checked-row toolbar). The confirm lists files plus an editable message. Ollama drafts the text if it is running locally; otherwise a fallback. Confirm is git add + git commit only — no push. CLI: `localhelm commit <id>... [--message TEXT] [--apply]`.
