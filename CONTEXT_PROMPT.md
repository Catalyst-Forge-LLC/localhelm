...
- **Land** (Sites + Today site cards, CLI `localhelm land <site-id>...`): named sites only — never an implicit land-all. Sites toolbar is **Land (N)** for checked rows and **Sync engine (N)** for checked rows that are behind; Today **Land N** / **Sync engine** cover sites that need an engine write. One confirm lists every site’s steps, then apply runs them in order and stops on first failure. Engine package writes (`filepress`), matching fleet package (`smellcheck-site` → `smellcheck`), then FilePress Sync → Push → Ship. Only already-needed writes. Publish OTP when a publish step is included. Never `--force`. Ship is skipped when the site tree fingerprint matches `.localhelm/land-ships.json` (recorded after a successful Land or Sites Ship). Land plan uses a one-pass FilePress `land` plan and status only for the engine/companion ids.
