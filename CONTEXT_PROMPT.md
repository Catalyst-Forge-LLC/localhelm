...
- Add projects hides folders that are already enrolled. The list is only new candidates. Scan shows the same Knight Rider bar as Confirm, with the live busy label. Nested FilePress `site/` folders are not scan candidates (enroll the package; the site lives under it).
