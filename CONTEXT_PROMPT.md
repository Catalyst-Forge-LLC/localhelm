...
- Looks cards group by title. One row lists every fact for that id (recipe, cwd, enroll vs lease). Enrolled Fleet/Sites ids with no exact lease name get **Enrolled, no port lease** / **Site has no port lease** (sibling `-site` does not count as claimed; parked leases still count). Hint still counts facts. Jumps follow the fact: recipe/split → Ports or Stacks, missing folder on an enrolled row → Fleet, lease with no fleet row → Add, enrolled with no lease → Ports. Claim stays on LocalSlip.
