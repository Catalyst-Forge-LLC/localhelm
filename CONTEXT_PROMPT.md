...
- After Push / Pull / Commit, the confirm stays open for a **git-only** status (`?git=1`): ahead/behind and dirty, no npm and no `commitsSinceNpm`. A long batch used to look stuck on “pushing N of N” while it re-checked npm (cache often expired mid-push) and ran pickaxe commit counts. The busy line switches to “reading git”. Merge keeps existing npm/pins.
